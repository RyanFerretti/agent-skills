...
> Source: Post by Thariq (@trq212) at Anthropic
...
A common misconception about skills is that they are "just markdown files", but the most interesting part is that they're not just text files. They're folders that can include scripts, assets, data, etc. that the agent can discover, explore and manipulate.

In Claude Code, skills also have a wide variety of configuration options including registering dynamic hooks.

Some of the most interesting skills use these configuration options and folder structure creatively.

---

## Types of Skills

After cataloging all of our skills, they cluster into a few recurring categories. The best skills fit cleanly into one; the more confusing ones straddle several. This isn't a definitive list, but it is a good way to think about if you're missing any inside of your org.

### 1. Library & API Reference

Skills that explain how to correctly use a library, CLI, or SDKs. These could be both for internal libraries or common libraries that Claude Code sometimes has trouble with. These skills often include a folder of reference code snippets and a list of gotchas for Claude to avoid when writing a script.
...
**Example pattern (standup-post/SKILL.md):**
```yaml
name: standup-post
...
```markdown
## Memory
Append each standup to ${CLAUDE_PLUGIN_DATA}/standups.log after posting. This folder persists across skill upgrades.
...
For example, in your data science skill you might have a library of functions to fetch data from your event source. Claude can then generate scripts on the fly to compose this functionality to do more advanced analysis for prompts like "What happened on Tuesday?"

**Example library (lib/signups.py):**
```python
def fetch(day):
    """Signups from events_raw for one day.
...
**Generated script (investigate.py — generated by Claude):**
```python
from lib.signups import fetch, by_referrer, by_landing_page

mon, tue = fetch("2024-03-11"), fetch("2024-03-12")

print(by_referrer(tue) - by_referrer(mon))    # organic -63%, paid flat
...
2. **Make a plugin** and have a Claude Code Plugin marketplace where users can upload and install plugins
...
Think of this more as a grab bag of useful tips that we've seen work than a definitive guide. The best way to understand skills is to get started, experiment, and see what works for you. Most of ours began as a single gotcha, and got better because people kept adding to them as Claude hit new edge cases.
